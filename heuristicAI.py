...
class HeuristicPlayer(BasePokerPlayer):
    def __init__(self, def_prob, agg=1):#def_prob=手役の強さに基づく行動確率,agg=攻撃性（ブラフやレイズ）
        """
    ...
    def update_fold_probability(self):
       """フォールド確率を0.1〜0.6の範囲でランダムに設定し、残りの行動確率を均等に再分配"""
       fold_prob = random.uniform(0.1, 0.6)
       remaining_prob = 1.0 - fold_prob
       other_probs = np.random.dirichlet(np.ones(3)) * remaining_prob  # レイズ、ブラフ、コールの確率の割合をランダムに決定
       self.default_prob[0] = [fold_prob, other_probs[0], other_probs[1], other_probs[2]]  # フォールド、レイズ、ブラフ、コールの順に設定
